...
//   onUploadFiles: (attachments: File[]) => void;
...
const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      ...
      onUploadFiles(files); // Trigger backend upload
    }
      ...
      <div className="w-full max-w-none mx-auto p-3 sm:p-4 md:p-6">
        {isRewriteMode && (
          <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center space-x-2">
              <Edit3 className="w-4 h-4 text-blue-600" />
              <span className="text-sm text-blue-800">Rewrite mode: Edit your prompt</span>
            </div>
          </div>
        )}
        ...
        {isUpdatingPrompt && (
          <div className="mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setIsUpdatingPrompt(false)}>
        ...
        {attachments.length > 0 && (
          <div className="mb-3 flex flex-wrap gap-2">
            {attachments.map((file, index) => (
              <Badge key={index} variant="secondary" className="flex items-center space-x-2 px-3 py-1 text-sm">
        ...
        {showGithubInput && (
          <div className="mb-3 flex items-center space-x-2">
            ...
            <Input
              ...
              className="flex-1"
            />
            <Button variant="outline" size="sm" onClick={handleGithubSubmit} disabled={!githubUrl.trim()}>
              Submit
            </Button>
            <Button variant="ghost" size="sm" onClick={() => setShowGithubInput(false)}>
              <X className="w-4 h-4" />
            </Button>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4">
          ...
          <div className="flex items-center space-x-2">
            {isStreaming && (
              <>
                <Button type="button" variant="outline" size="sm" onClick={onStopAndRewrite}>
                  <Edit3 className="w-4 h-4" />
                </Button>
                <Button type="button" variant="outline" size="sm" onClick={onStopChat}>
                  <Square className="w-4 h-4" />
                </Button>
              </>
            )}
            ...
            {!isStreaming && !isUpdatingPrompt && !isRewriteMode && (
              <Button type="button" variant="outline" size="sm" onClick={() => setIsUpdatingPrompt(true)}>
                Edit
              </Button>
            )}
            ...
            >
              <Send className="w-4 h-4" />
              <span className="ml-1 hidden sm:inline">{isRewriteMode ? 'Rewrite' : 'Send'}</span>
            </Button>
          </div>
        </form>
